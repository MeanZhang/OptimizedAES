...
public class AES128 {
     ...
     * T盒
     * <p>
     * 加密或解密过程中用于实现字节代替、行移位和列混淆的查找表
     */
    private byte[][][] T = new byte[4][256][4];
    /**
     * 11个子密钥
     * <p>
     ...
     * @param mode 模式，加密或解密
     */
    public AES128(byte[] key, String mode) {
        init(key, mode);
    }
    ...
    private void keyExpansion(byte[] key) {
        ...
        for (int i = 4; i < 44; i++) {
            // 如果i是4的倍数，wi = wi-4 ⊕ g(wi-1)，g为循环左移1字节⊕字节代替⊕Rcon
            if (i % 4 == 0) {
                // 异或Rcon即第一个字节异或RC
     ...
     * @param mode 模式，加密或解密
     */
    public void init(byte[] key, String mode) {
        // 判断模式是否正确
        ...
        if (!mode.equals(this.mode)) {
            ...
            if (mode.equals("encrypt")) {
                matrix = new byte[][]{{2, 1, 1, 3}, {3, 2, 1, 1}, {1, 3, 2, 1}, {1, 1, 3, 2}};
                // 构造T盒
                for (int i = 0; i < 4; i++)
                    for (int j = 0; j < 256; j++)
                        for (int k = 0; k < 4; k++)
                            T[i][j][k] = multiply(S[j], matrix[i][k]);
            } else {
                matrix = new byte[][]{{0xE, 0x9, 0xD, 0xB}, {0xB, 0xE, 0x9, 0xD}, {0xD, 0xB, 0xE, 0x9},
                        {0x9, 0xD, 0xB, 0xE}};
                for (int i = 0; i < 4; i++)
                    for (int j = 0; j < 256; j++)
                        for (int k = 0; k < 4; k++)
                            T[i][j][k] = multiply(INV_S[j], matrix[i][k]);
            }
        }
    }
     ...
     * @return 状态矩阵
     */
    private byte[][] roundFunction(byte[][] state, byte[][] key) {
        byte[][] result = new byte[4][4];
        for (int i = 0; i < 4; i++)
            // state[i] = T0(state[i][0]) + T1(state[(i+1)%4][1]) + T2(state[(i+2)%4][1]) +
            // T3(state[(i+3)%4][1]) + k[i]
            for (int j = 0; j < 4; j++)
                result[i][j] = (byte) (T[0][state[i][0] & 0xff][j] ^ T[1][state[(i + 1) % 4][1] & 0xff][j]
                        ^ T[2][state[(i + 2) % 4][2] & 0xff][j] ^ T[3][state[(i + 3) % 4][3] & 0xff][j] ^ key[i][j]);
        return result;
    }
     ...
     * 逆轮函数
     * <p>
     * 逆字节代替、逆行移位和逆列混淆的查表实现
     *
     * @param state 状态矩阵
     * @param key   轮密钥
     * @return 状态矩阵
     */
    private byte[][] invRoundFunction(byte[][] state, byte[][] key) {
        byte[][] result = new byte[4][4];
        for (int i = 0; i < 4; i++)
            // state[i] = T0(state[i][0]) + T1(state[(i+3)%4][1]) + T2(state[(i+2)%4][1]) +
            // T3(state[(i+1)%4][1]) + k[i]
            for (int j = 0; j < 4; j++)
                result[i][j] = (byte) (T[0][state[i][0] & 0xff][j] ^ T[1][state[(i + 3) % 4][1] & 0xff][j]
                        ^ T[2][state[(i + 2) % 4][2] & 0xff][j] ^ T[3][state[(i + 1) % 4][3] & 0xff][j] ^ key[i][j]);
        return result;
    }
